...
const filterWithoutSift = (filters, nodeTypeNames, filtersCache) => {
  ...
  nodesPerValueArrs.sort((a
  /*: Array<IGatsbyNode> */
  , b
  /*: Array<IGatsbyNode> */
  ) => b.length - a.length); // All elements of nodesPerValueArrs should be sorted by counter and deduped
  // So if there's only one bucket in this list the next loop is skipped

  while (nodesPerValueArrs.length > 1) {
    nodesPerValueArrs.push(intersectNodesByCounter(nodesPerValueArrs.pop(), nodesPerValueArrs.pop()));
  }
...
const getBucketsForQueryFilter = (filterCacheKey, filter, nodeTypeNames, filtersCache, nodesPerValueArrs) => {
  let {
    path: filterPath,
    query: {
      comparator
  ...
  = getNodesFromCacheByValue(filterCacheKey, filterValue, filtersCache, false); // If we couldn't find the needle then maybe sift can, for example if the
...
const collectBucketForElemMatch = (filterCacheKey, filter, nodeTypeNames, filtersCache, nodesPerValueArrs) => {
  ...
  const nodesByValue
  /*: Array<IGatsbyNode> | undefined*/
  = getNodesFromCacheByValue(filterCacheKey, targetValue, filtersCache, true); // If we couldn't find the needle then maybe sift can, for example if the
  // schema contained a proxy; `slug: String @proxy(from: "slugInternal")`
  // There are also cases (and tests) where id exists with a different type
...
const applyFilters = (filterFields, firstOnly, nodeTypeNames, filtersCache, resolvedFields, stats) => {
  ...
  lastFilterUsedSift = true; // const siftResult /*: Array<IGatsbyNode> | null */ = filterWithSift(
  //   filters,
  //   firstOnly,
  //   nodeTypeNames,
  //   resolvedFields
